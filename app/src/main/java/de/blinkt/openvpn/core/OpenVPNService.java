...
import static com.android.bytesbee.vpnapp.managers.UsageManager.KEY_TOTAL_TIME;
import static com.android.bytesbee.vpnapp.managers.UsageManager.STR_TIME;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel, long when, ConnectionStatus status, Intent intent) {
        ...
        //Commented by Prashant for Disconnected button, pause, and resume button
...
//        sp_settings = getSharedPreferences(PREF_USAGE, 0);
        long_usage_today = UsageManager.get().getUsage(TODAY); //sp_settings.getLong(TODAY, 0);
        long_usage_week = UsageManager.get().getUsage(WEEK + YEAR); // sp_settings.getLong(WEEK + YEAR, 0);
        long_usage_month = UsageManager.get().getUsage(MONTH + YEAR); // sp_settings.getLong(MONTH + YEAR, 0);
        long_usage_time_today = UsageManager.get().getUsage(TODAY + STR_TIME); // sp_settings.getLong(TODAY + "_time", 0);
        long_usage_time_total = UsageManager.get().getUsage(KEY_TOTAL_TIME); // sp_settings.getLong("total_time", 0);
            ...
            countDownTimer = new CountDownTimer(86400_000, 1000) {
                public void onTick(long millisUntilFinished) {
                    ...
                    if ((long_milli_seconds % time) == 0) {
                        UsageManager.get().setUsage(TODAY, long_usage_today + long_usage_now);
                        UsageManager.get().setUsage(WEEK + YEAR, long_usage_week + long_usage_now);
                        UsageManager.get().setUsage(MONTH + YEAR, long_usage_month + long_usage_now);
                        UsageManager.get().setUsage(TODAY + STR_TIME, long_usage_time_today + long_milli_seconds);
                        UsageManager.get().setUsage(KEY_TOTAL_TIME, long_usage_time_total + long_milli_seconds);
                    }
